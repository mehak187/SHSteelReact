...
const CapacityCalculator = () => {
  ...
  return (
    <div>
      <div className="2xl:flex mt-5 gap-12">
        <div className="2xl:w-[40%]">
          <div className="flex justify-between items-center">
            <div>
              <h4 className="font-medium sm:text-3xl text-xl">
                Capacity Calculator
              </h4>
            </div>
            <div>
            ...
            <table className="w-full">
              <thead className="border-b-4">
                <tr className="align-bottom text-left ">
                  <th
                    rowSpan={3}
                    className="p-3 border-r-8 border-[#F4F5FA] bg-white"
                  >
                    Structural
                  </th>
                </tr>
                <tr>
                  <th colSpan={2} className="p-3 border-b-4 bg-white">
                    Hrs/week
                  </th>
                </tr>
                <tr>
              ...
              <tbody>
                {structuralData.map((row, index) => (
                  <tr key={index}>
                    <td className="p-3 text-[#828280] bg-white border-r-8 border-[#F4F5FA]">
                      {row.label}
                    </td>
                    <td className="text-center p-3 text-[#828280] bg-white">
                      {row.emp}
                    </td>
                    <td className="text-center p-3 text-[#828280] bg-white">
                      {row.hours}
                ...
                <tr className="border-t-4 border-[#000]">
                  <td className="p-3 text-[#000] font-medium bg-white border-r-8 border-[#F4F5FA]">
                    Total
                  </td>
                  <td className="text-center p-3 text-[#000] font-medium bg-white">
                    14
                  </td>
                  <td className="text-center p-3 text-[#000] bg-white font-medium">
                    560
                  </td>
                </tr>
                <tr>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-4 border-[#000]">
                  <th className="p-3 bg-white text-left border-r-8 border-[#F4F5FA] w-[500px]">
                    Misc
                  </th>
              ...
              <tbody>
                {miscData.map((row, index) => (
                  <tr key={index}>
                    <td
                      className={`p-3 text-[#828280] border-r-8 border-[#F4F5FA] bg-white ${
                        row.label === "Total" ? "border-t-4 border-[#000]" : ""
                      }`}
                    >
                      {row.label}
                    </td>
                    <td
                      className={`text-center p-3 text-[#828280] bg-white ${
                        row.label === "Total" ? "border-t-4 border-[#000]" : ""
                      }`}
                    >
                      {row.emp}
      ...
      <div className="mt-7">
        <h4 className="bg-[#88191F] text-white font-medium text-3xl py-2 px-4 w-full">
          Metrics
        </h4>
          ...
          <table className="w-full">
            ...
            <tbody>
              {annualData.map((row, index) => (
                <tr key={index}>
                  <td
                    className={`p-3 text-[#828280] bg-white ${
                      row.label === "Total" ? "border-t-4 border-[#000]" : ""
                    }`}
                  >
                    {row.label}
                  ...
                  <td
                    className={`text-center p-3 text-[#828280] bg-white inset-shadow-sm ${
                      row.label === "Total" ? "border-t-4 border-[#000]" : ""
                    }`}
                  >
